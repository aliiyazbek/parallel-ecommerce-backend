...
|-------|--------:|---------------:|--------:|---------:|----------:|---------:|
| **BEFORE - single node** | 1 | 8 | 184 | 116 | 1250 | 51 |
| **AFTER - 4 nodes (round_robin)** | 4 | 32 | 300 | 0 | 1260 | 51 |

**Verdict:** scaling from 1 node to 4 nodes behind the load balancer cut rejected requests from **116** to **0** (116 fewer). A single node tops out at its worker limit and drops the overflow; spreading the same traffic across nodes removes that single-instance bottleneck.
